...
// Wir erstellen das ManualOperationRectangle
draw2d.shape.flowchart.ManualOperationRectangle = draw2d.VectorFigure.extend({
 
 NAME: 'draw2d.shape.flowchart.ManualOperationRectangle',
 ...
 repaint: function(attributes) {
  ...
  path.push('L', box.x + 120.5 * factorX, ' ', box.y + 60.5 * factorY);
  path.push('L', box.x + 20.5 * factorX, ' ', box.y + 60.5 * factorY);
  path.push('L', box.x + 0.5 * factorX, ' ', box.y + 0.5 * factorY);
  //path.push('L', box.x + 140.5, ' ', box.y + 0.5);
  path.push('Z');
...
draw2d.shape.flowchart.ManualOperationPorts = draw2d.shape.flowchart.ManualOperationRectangle.extend({

 NAME: 'draw2d.shape.flowchart.ManualOperationPorts',
...
// FlowChart - ManualOperation erstellen
draw2d.shape.flowchart.ManualOperation = draw2d.shape.flowchart.ManualOperationPorts.extend({

 NAME: 'draw2d.shape.flowchart.ManualOperation',

	init: function(attr, setter, getter) {
  this._super(attr, setter, getter);
  this.classLabel = new draw2d.shape.basic.Label({
   stroke: 1,
   text: LangVar['txt_ManualOperation'],
   fontSize: 16,
   color: "#",
   fontColor: "#303030",
   bgColor: "#",
  ...
  var box = this.getBoundingBox();
  this.add(imgSymbol, new draw2d.shape.flowchart.locationXY(box.w - 26, 4)); 
